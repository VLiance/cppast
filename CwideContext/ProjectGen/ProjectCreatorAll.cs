...
{
    class ProjectCreatorAll : FileUtils
    {
       ...
       // StreamWriter writer;
        ...
        public ProjectCreatorAll(Project _oProject ,string _sGroupList, string _sProjectName,  string _sOutputDir, string _sOutBranch, List<string> _aFileList, List<string> _aIncludeDir, string _sPlatform, string _sOpLibList)
        {
			oProject = _oProject;
			
			
            
            string _sPrjFile = _sOutputDir + _sOutBranch + "Prj_"+ _sProjectName + "All.cbp";
...
/*
				...
				//	fAdd("<Add option='-lgdi32' />");
				//	fAdd("<Add option='-lopengl32' />");

				fSubTab();
				fAdd("</Linker>");

				fAdd("<Unit filename='" + _sProjectName + "_BuildAll.cpp" + "' />");
