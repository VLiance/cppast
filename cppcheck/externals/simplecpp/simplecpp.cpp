...
printf("\n---!!--sourcefile:%s\n",sourcefile.c_str());
	...
	//	printf("--*IncludSourcefilee:%s\n",header.c_str());
        ...
        if (hasFile(ret, sourcefile, header, dui, systemheader))  //If already in our list
            continue;
...
printf("-------------------------PREPROCESS TOKEN %s\n ","a" );
    for (const Token *rawtok = NULL; rawtok || !includetokenstack.empty();) {
        if (rawtok == NULL) {  //Reusse INCLUDE tokens
...
//	printf("--IncludSourcefilee:%s\n",header.c_str());
                ...
                } else if (includetokenstack.size() >= 400) { //EROOR too deep stack
                    if (outputList) {
                        simplecpp::Output out(files);
...
printf("------------------------ENDDDDDDDDDDDDDDDD-----PREPROCESS TOKEN %s\n ","a" );
