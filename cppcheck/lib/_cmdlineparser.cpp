...
char aInputBuff[1000];
 char* sNull = "\0";
#define Is_Char(_cVal) ( (_cVal >= 'A' && _cVal <= 'Z')  || (_cVal >= 'a' && _cVal <= 'z')  ||  (_cVal >= '0' && _cVal <= '9') || _cVal == '_'  || _cVal == '\"' || _cVal == '\'' || _cVal == '\\' || _cVal == '/' || _cVal == ':' )
...
//	printf("Cmd: %s\n", _sCmd);
	...
	fTestCmdVal(_sCmd,_sVal1, _sVal2,_sVal3 );
}
...
void CmdLineParser::fTestCmdVal(const char* _sCmd, const char* _sVal1, const char* _sVal2, const char* _sVal3)
{
 ...
 _settings->experimental = false;
  ...
  _settings->dump = true;
		...
		//	fAddDefine("__llvm__=1");
			...
			///////Include ///
			fAddSource("E:/_Project/Cwc/_Cwc_Demos/Demos/Base_Example/01_HelloWorld/HelloWorld.cpp");
			//////////////////////
...
/*
		...
		int _nTest = 0;
		cppexecutor->fRun();
		cppexecutor->check_wrapper(*cppcheck, _nTest, &ExeLocation);
