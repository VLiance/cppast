...
#include "ObjFile.h"
//std::map<std::string, Preprocessor> aFileData;
std::map<std::string, ObjFile*> aFileData;
...
ObjFile*  CppCheck::fReadFile(const std::string& filename){
	...
	////////////////////////
	  ...
	  try {
		...
		for (unsigned int i = 0; i <  _oObjFile->oPreproc->aIncludesList.size(); ++i) {
			fReadFile(_oObjFile->oPreproc->aIncludesList[i]);
		}
	...
	//	Preprocessor* preprocessor =
        ...
        configurations.insert(_settings.userDefines);  //Get user define!
...
/*
        ...
        for (std::set<std::string>::const_iterator it = configurations.begin(); it != configurations.end(); ++it) { //Only one?
            ...
            Tokenizer _tokenizer11(&_settings, this);


//Tokenizer _tokenizerOld =  &_oObjFile->oTokenizer;
//Tokenizer* _tokenizer = &_tokenizer11;
Tokenizer* _tokenizer =  _oObjFile->oTokenizer;
...
*/
				...
				///All other files
				   for (unsigned int i = 0; i <  preprocessor->aIncludesList.size(); ++i) {
					printf("\n-------files: %s",  preprocessor->aIncludesList[i].c_str());
				}
        ...
        exitcode=1; // e.g. reflect a syntax error-r
    }
	return 0;
}
...
/*
	///All other files
	for (unsigned int i = 0; i <  _oFile->aIncludesList.size(); ++i) {
	//	printf("\n-------files: %s",  _oFile->aIncludesList[i].c_str());
		fReadFile(_oFile->aIncludesList[i]);
	}*/




	//fReadFile( filename, cfgname, fileStream);

return 0;//CW-


std::string  filenameAss ="E:/_Project/Cwc/_Cwc_Demos/Demos/Base_Example/01_HelloWorld/HelloWorld.cpp";


   exitcode = 0;

    // only show debug warnings for accepted C/C++ source files
    if (!Path::acceptFile(filenameAss))
    ...
    if (_settings.quiet == false) {
        std::string fixedpath = Path::simplifyPath(filenameAss);
        fixedpath = Path::toNativeSeparators(fixedpath);
    ...
    try {
        ...
        simplecpp::OutputList outputList;
        std::vector<std::string> files;
        simplecpp::TokenList tokens1(fileStream, files, filenameAss, &outputList);
        preprocessor.loadFiles(tokens1, files);
/*
        ...
        if (!_settings.buildDir.empty()) {
           ...
           const unsigned int checksum = preprocessor.calculateChecksum(tokens1, toolinfo);
            ...
            if (!analyzerInformation.analyzeFile(_settings.buildDir, filename, cfgname, checksum, &errors)) {
               ...
               // return exitcode;  // known results => no need to reanalyze file  ///CW- Stupid stop here
            }
        }
*/
        // Get directives
...
/*
        ...
        if (_settings.userDefines.empty() || _settings.force) {
            ...
            configurations = preprocessor.getConfigs(tokens1); //Process absolutly all config
        } else {*/
            configurations.insert(_settings.userDefines);  //Get user define!
    //    }

	//	printf("----------!!USER DEFINE!!: %s \n", _settings.userDefines);
/*
        if (_settings.checkConfiguration) {
           ...
           // return 0; //CW- asshole stoping heree
        }*/
/*
...
*/
        ...
        std::ofstream fdump;
        if (_settings.dump) {
            const std::string dumpfile(filenameAss + ".dump");
            fdump.open(dumpfile.c_str());
            if (fdump.is_open()) {
                fdump << "<?xml version=\"1.0\"?>" << std::endl;
                fdump << "<dumps>" << std::endl;
...
//files[header2]
            ...
            codeWithoutCfg += _settings.append();
		//	printf("--codeWithoutCfg: %s\n" ,codeWithoutCfg.c_str());
/*
            ...
            try {
                ...
                std::istringstream istr(codeWithoutCfg);
                Timer timer("Tokenizer::createTokens", _settings.showtime, &S_timerResults);
                bool result = _tokenizer.createTokens(istr, filenameAss);
                timer.Stop();
...
///////Process others
			 for (unsigned int i = 0; i < preprocessor.aIncludesList.size(); ++i) {
					printf("\-----MAIN----Files: %s \n", preprocessor.aIncludesList[i].c_str());
				//	fReadFile(files[i], cfgname, fileStream);
				}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                // Analyze info..
                if (!_settings.buildDir.empty())
                    checkUnusedFunctions.parseTokens(_tokenizer, filenameAss.c_str(), &_settings, false);
                ...
                if (e.token) {
                    ...
                    ErrorLogger::ErrorMessage::FileLocation loc2;
                    loc2.setfile(Path::toNativeSeparators(filenameAss));
                    locationList.push_back(loc2);
                    loc.setfile(_tokenizer.list.getSourceFilePath());
                }
                locationList.push_back(loc);
        ...
        internalError(filenameAss, e.what());
    } catch (const std::bad_alloc &e) {
        internalError(filenameAss, e.what());
    } catch (const InternalError &e) {
        internalError(filenameAss, e.errorMessage);
    ...
    if (!_settings.jointSuppressionReport && (_settings.isEnabled("information") || _settings.checkConfiguration)) {
        reportUnmatchedSuppressions(_settings.nomsg.getUnmatchedLocalSuppressions(filenameAss, isUnusedFunctionCheckEnabled()));
    }
