...
public:
	...
	void fParseLine(const char * _sLine);

	void fParseCmd(const char * _sLine);

	void fParseVal(const char * _sCmd);

	void fAddInclude(std::string _path);


	CppCheck* cppcheck;
CppCheckExecutor* cppexecutor;

	/**
     * Parse given command line.
     * @return true if command line was ok, false if there was an error.
     */
void fAddSource(std::string _path);
void fAddDefine(std::string _define);
void fAddEnable(std::string);
void fTestCmdVal(const char * _sArg, const char * _sVal1, const char * _sVal2, const char * _sVal3, unsigned int _nLength1, unsigned int _nLength2, unsigned int _nLength3);
...
protected:
     ...
     */
	bool ParseFromArg(const char * argv, const char * aNext);
