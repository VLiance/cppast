...
parser.cppcheck = cppcheck;

parser.cppexecutor = this;
...
bool CppCheckExecutor::fRun(){

CppCheck *cppcheck = parser.cppcheck;
  Settings& settings = cppcheck->settings();
 ...
 *
 */
int CppCheckExecutor::check_wrapper(std::string _sFile,CppCheck& cppcheck, int argc, const char* const argv[])
{
#ifdef USE_WINDOWS_SEH
    FILE *outputFile = stdout;
    __try {
    ...
    return check_internal(cppcheck, argc, argv);
#else
    return check_internal(_sFile,cppcheck, argc, argv);
#endif
}
 ...
 * */
int CppCheckExecutor::check_internal(std::string _sFile, CppCheck& cppcheck, int /*argc*/, const char* const argv[])
{
    Settings& settings = cppcheck.settings();
    _settings = &settings;
