...
{
   // return !Path::isHeader(path) && (Path::isCPP(path) || Path::isC(path) || extra.find(getFilenameExtension(path)) != extra.end()); //CW-   -> extra buuuuuuuuug
 //   return !Path::isHeader(path) && (Path::isCPP(path) || Path::isC(path) ); //CW- --> Accept all files
    return Path::isHeader(path) || (Path::isCPP(path) || Path::isC(path) ); //CW- --> Accept all files
//	return true;
}
