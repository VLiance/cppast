...
printf("c3\n");
 ...
 //   simplifyCAlternativeTokens(); //Good?
    ...
    prepareTernaryOpForAST();   //Usefull? in double below
...
//#endif
    ...
    if (m_timerResults) {
        ...
        setVarId();   ////Set variable heeeeeeerre
    }



    // Link < with >
    createLinks2(); //Important
...
void Tokenizer::syntaxError(const Token *tok) const
{
    ...
    return;//CW mod
       ...
       //        reportError(tok, Severity::debug, "debug", "simplifyKnownVariables: bailing out (variable="+tok->str()+", value="+value+")");
